    ...
    stage('Get Dockerfile From Version Control') {
      steps {
        ...
        container(name: 'kaniko', shell: '/busybox/sh') {
          ...
          sh ''
          '#!/busybox/sh /
          kaniko / executor--context `pwd`--skip - tls - verify--no - push - c / workspace--dockerfile $ {
            WORKSPACE
          }
          /metadata/public.Dockerfile--destination = pipeline - primary: debug--tarPath = $ {
            WORKSPACE
          }
          /image/amundsen - metadata.tar ''
          '
          //  dir('definitions') {
          //     script 
          //     {
