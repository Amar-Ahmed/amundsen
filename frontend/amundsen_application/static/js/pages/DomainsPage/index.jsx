...
export class DomainsPage extends Component {
  ...
  componentDidMount() {
    getDomains().then(response => this.setState({ domains: response })).catch(err => console.log(err))
  }
  ...
  renderDomainList = () => {
    return this.state.domains.map(domain => {
      if (domain.domain_data_asset) {
        return (<tr>
          <td className='title-3'>
            <a className='capitalize'
              href={`/domains/${domain.domain_name}/`}>{domain.domain_name}
            </a>
          </td>
        ...
        return (<tr>
          <td className='title-3'><span className='capitalize'>
            {domain.domain_name}
          </span></td>
          <td className='domains-list-description-container'>
            <Truncate text={domain.domain_description} />
            ...
            <div className="col-xs-12 col-md-10 col-md-offset-1">
              ...
              <div className="row " style={{ marginTop: '24px' }}>
                <Table responsive>
                  <thead>
                    <tr>
                      <th className='title-3' style={{ minWidth: '170px' }}>Name</th>
                      <th className='title-3' style={{ maxWidth: '170px' }}>Description</th>
                    </tr>
                  </thead>
                  <tbody>
